...
# and DiT_Graphormer by Meta AI (Peebles and Xie): https://github.com/facebookresearch/DiT_Graphormer
...
class _DiTNoiseNet(nn.Module):
    ...
    def forward(
        ...
        return enc_cache, self.forward_dec(
            noise_actions, time, enc_cache, degree_matrix, distance_matrix
        )
    ...
    def forward_dec(
        self, noise_actions, time, enc_cache, degree_matrix=None, distance_matrix=None
    ):
        time_enc = self.time_net(time)
        B = noise_actions.shape[0]

        degree_matrix = degree_matrix.repeat(B, 1)
        spatial_pos = distance_matrix.unsqueeze(0).repeat(B, 1, 1)
