...
def _get_imgs(t, cam_idx, past_frames, moge=False):
    imgs = []
    while len(imgs) < past_frames + 1:
        if moge:
            ...
            if cam_idx == 0:
                img_data = t.obs.image(0)
            elif cam_idx == 1:
                img_data = t.obs.depth(0)
            elif cam_idx == 2:
                img_data = t.obs.normal(0)
            elif cam_idx == 3:
                img_data = t.obs.mask(0)
            else:
                raise ValueError("Invalid cam_idx for moge")
            imgs.append(img_data[None])
        else:
...
class RobobufReplayBuffer(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for idx, cam_idx in enumerate(self.cam_indexes):
            i_c = _get_imgs(step, cam_idx, self.past_frames, self.moge)
            if self.goal_indexes:
                g_c = (
                    _get_imgs(goal, cam_idx, 0, self.moge)
